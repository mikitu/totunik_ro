...
import { Button, Modal, TextInput, Grid, GridItem, Box, Typography, Flex } from '@strapi/design-system';
import { Search, Plus, Cross } from '@strapi/icons';

const ICON_LIST = [
  'none', 'arrow-right', 'arrow-left', 'arrow-up', 'arrow-down', 
  'check', 'close', 'plus', 'minus', 'search', 'filter', 
  'edit', 'delete', 'save', 'download', 'upload', 'share', 
  'heart', 'star', 'bookmark', 'home', 'user', 'users', 
  'settings', 'bell', 'mail', 'phone', 'calendar', 'clock', 
  'map', 'location', 'camera', 'image', 'video', 'music', 
  'file', 'folder', 'link', 'external-link', 'eye', 'eye-off', 
  'lock', 'unlock', 'shield', 'info', 'warning', 'error', 'success'
];
...
const IconPreview = ({ iconName }: { iconName: string }) => {
  if (!iconName || iconName === 'none') return <div style={{ width: 24, height: 24 }} />;
...
export const IconSelector: React.FC<IconSelectorProps> = ({
  ...
  return (
    <Box>
      ...
      {isOpen && (
        <Modal.Root defaultOpen onOpenChange={setIsOpen}>
          <Modal.Content>
            <Modal.Header>
            ...
            <Modal.Body>
              ...
              <Grid gap={2} style={{ maxHeight: '400px', overflow: 'auto' }}>
                {filteredIcons.map((iconName) => (
                  <GridItem key={iconName} col={2}>
                    <Button
                      variant={value === iconName ? "primary" : "secondary"}
                    ...
                    >
                      ...
                      <Typography variant="omega" style={{ fontSize: '10px' }}>
                        {iconName}
                      </Typography>
                    </Button>
                  </GridItem>
            ...
            <Modal.Footer>
              ...
              <Button onClick={() => handleIconSelect('none')}>
                Clear Selection
              </Button>
            </Modal.Footer>
          </Modal.Content>
