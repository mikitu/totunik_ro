...
export default async function ContactPage({ params }: ContactPageProps) {
  ...
  return (
    <main className="font-sans text-gray-800">
      ...
      <section className="py-20 bg-gray-50">
        <div className="container mx-auto px-6">
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-16">
            {/* Contact Information */}
            {contactData?.ContactInfo && <ContactInfo contactInfo={contactData.ContactInfo} />}
      ...
      {/* Sales Team */}
      {contactData?.SalesTeam && <SalesTeam salesTeam={contactData.SalesTeam} />}

      {/* Contact Map */}
      {contactData?.Map && <ContactMap mapSection={contactData.Map} />}
      <Footer />
    </main>
  );
}
