  ...
  return (
    <footer className="bg-gray-900 text-white py-12 px-6">
      <div className="container mx-auto grid md:grid-cols-4 gap-10">
        {/* Logo & description */}
        <div>
          {footer?.logo && <Image src={footer.logo.url} alt="Logo" width={120} height={40} />}
          {footer?.description && <p className="mt-4 text-gray-400">{footer.description}</p>}
        </div>

        {/* Link Groups */}
        {Array.isArray(footer?.navigation) &&
          footer.navigation.map((group, idx: number) => (
            <div key={idx}>
