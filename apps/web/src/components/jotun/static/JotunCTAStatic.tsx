...
export default function JotunCTAStatic() {
  ...
  return (
    <section ref={ref} className="relative py-20 bg-gradient-to-br from-blue-900 via-blue-800 to-blue-700 overflow-hidden">
      {/* Background Pattern */}
      <div className="absolute inset-0 opacity-10">
        <div className="absolute top-20 right-20 w-64 h-64 bg-white rounded-full blur-3xl animate-pulse" />
        <div className="absolute bottom-20 left-20 w-96 h-96 bg-orange-500 rounded-full blur-3xl animate-pulse" style={{ animationDelay: '1s' }} />
        <div className="absolute top-1/2 right-1/4 w-32 h-32 bg-white rounded-lg rotate-45 blur-2xl animate-pulse" style={{ animationDelay: '2s' }} />
      </div>
      ...
      <div className="container mx-auto px-6 relative z-10">
        <div className="max-w-6xl mx-auto">
          ...
          <div className="text-center mb-16">
            <h2 className={`text-4xl md:text-5xl lg:text-6xl font-bold text-white mb-6 leading-tight transition-all duration-1000 ${isVisible ? 'animate-fadeInUp' : 'opacity-0 translate-y-8'}`}>
              Ready to Transform Your Project?
            </h2>
            <p className={`text-xl md:text-2xl text-blue-100 mb-12 max-w-3xl mx-auto transition-all duration-1000 ${isVisible ? 'animate-fadeInUp' : 'opacity-0 translate-y-8'}`} style={{ animationDelay: '200ms' }}>
              Partner with Totunik and Jotun for premium coatings that deliver exceptional results
            </p>

            {/* CTA Buttons */}
            <div className={`flex flex-col sm:flex-row gap-6 justify-center mb-16 transition-all duration-1000 ${isVisible ? 'animate-fadeInUp' : 'opacity-0 translate-y-8'}`} style={{ animationDelay: '400ms' }}>
