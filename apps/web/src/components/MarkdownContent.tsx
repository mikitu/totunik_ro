...
export default function MarkdownContent({
  ...
  const getComponents = () => {
    const baseComponents = {
      ...
      // Text formatting
      strong: ({ children }: any) => (
        <strong className={`font-bold ${variant === 'white' ? 'text-white' : 'text-gray-900'}`}>
          {children}
        </strong>
      ),
      em: ({ children }: any) => (
        <em className={`italic ${variant === 'white' ? 'text-white/95' : 'text-gray-700'}`}>
          {children}
        </em>
      ),
      ...
      code: ({ children }: any) => (
        <code
          className={`px-2 py-1 rounded text-sm font-mono ${
            variant === 'white' ? 'bg-white/20 text-white' : 'bg-gray-100 text-gray-800'
          }`}
        >
          {children}
        </code>
  ...
  return (
    <div className={`prose max-w-none ${getVariantClasses()} ${className}`}>
      <ReactMarkdown components={getComponents()}>{content}</ReactMarkdown>
    </div>
  );
}
